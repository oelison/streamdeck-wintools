...
namespace VirtualDesktop
{
	...
	*/
    ...
    #region Windows Support
	internal static class WindowsHelpers
    {
		private const string REG_WINDOWS_VERSION_KEY = @"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion";
		private const string REG_RELEASE_VALUE = @"ReleaseId";
		private const int INVALID_VERSION = -1;
		private const int MINIMAL_VIRTUAL_DESKTOP_VERSION = 2004;

		private static int? prefetchedReleaseVersion = null;

		public static bool IsSupportedVirtualDesktopVersion()
        {
			return GetWindowsReleaseVersion() >= MINIMAL_VIRTUAL_DESKTOP_VERSION;
        }

		static int GetWindowsReleaseVersion()
        {
			try
            {
				if (prefetchedReleaseVersion != null)
                {
					return prefetchedReleaseVersion.Value; ;
                }

				string value = (string) Registry.GetValue(REG_WINDOWS_VERSION_KEY, REG_RELEASE_VALUE, null);
				if (String.IsNullOrEmpty(value))
				{
					Logger.Instance.LogMessage(TracingLevel.ERROR, $"GetWindowsReleaseVersion failed to get ReleaseId!");
					prefetchedReleaseVersion = INVALID_VERSION;
					return prefetchedReleaseVersion.Value;
				}
				if (!Int32.TryParse(value, out int releaseVersion))
                {
					Logger.Instance.LogMessage(TracingLevel.ERROR, $"GetWindowsReleaseVersion ReleaseId is invalid: {value}");
					prefetchedReleaseVersion = INVALID_VERSION;
					return prefetchedReleaseVersion.Value;
				}
				Logger.Instance.LogMessage(TracingLevel.INFO, $"GetWindowsReleaseVersion ReleaseId: {value}");
				prefetchedReleaseVersion = releaseVersion;
				return prefetchedReleaseVersion.Value;
			}
			catch (Exception ex)
            {
				Logger.Instance.LogMessage(TracingLevel.ERROR, $"GetWindowsReleaseVersion Exception {ex}");
            }
			prefetchedReleaseVersion = INVALID_VERSION;
			return prefetchedReleaseVersion.Value;
		}
