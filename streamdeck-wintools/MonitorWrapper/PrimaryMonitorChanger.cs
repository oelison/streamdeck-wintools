...
namespace WinTools.MonitorWrapper
{
    public class PrimaryMonitorChanger
    {
        public static bool SetAsPrimaryMonitor(Screen destinationScreen)
        {
            ...
            uint id;
            for (id = 0; NativeMethods.EnumDisplayDevices(null, id, ref device, 0); id++)
            {
                if (device.DeviceName == destinationScreen.DeviceName)
                {
                    break;
                }
            }
            ...
            if (device.DeviceName != destinationScreen.DeviceName)
            {
                Logger.Instance.LogMessage(TracingLevel.ERROR, $"EnumDisplayDevices did not return the screen: {destinationScreen.DeviceName}");
                return false;
            }
